...
export class StatsService extends Effect.Service<StatsService>()("StatsService", {
    ...
    return {
      getPlayerStats: (input: GetPlayerStatsInput) =>
        Effect.gen(function* () {
          ...
          Effect.tap(() => Effect.log(`Fetched stats for player ${input.playerId}`)),
      ...
      getTeamStats: (input: GetTeamStatsInput) =>
        Effect.gen(function* () {
          ...
          Effect.tap(() => Effect.log(`Fetched stats for team ${input.teamId}`)),
          Effect.tapError((error) =>
